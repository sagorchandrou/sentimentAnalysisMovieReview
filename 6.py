...
def remove_stopword(text):
    ...
    #sentence = text
    stopwords_english_for_bigrams = set(stopwords.words("english")) - set(important_words)
    words = nltk.word_tokenize(text)
    without_stop_words = [word for word in words if not word in stopwords_english_for_bigrams]
...
def write_fileOut(child1,child2):
    with open('D:/finalworkToday/traintest.csv','a') as fd:
...
with open('D:/finalworkToday/IMDB.csv',mode='r',errors='ignore',encoding="utf8") as csv_file1:
    ...
    for row in reader:
        ...
        text_lemalized=word_streaming(text_clean)
...
trainTestData = pd.read_csv('D:/finalworkToday/traintest.csv')
